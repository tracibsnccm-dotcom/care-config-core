...
-- Insert default retention policy (only if table is empty)
INSERT INTO public.data_retention_policy (id, client_record_years, purged_data_backup_days, export_before_purge)
SELECT gen_random_uuid(), 7, 30, true
WHERE NOT EXISTS (SELECT 1 FROM public.data_retention_policy);
...
DROP POLICY IF EXISTS "Staff can view attorney SLA" ON public.attorney_sla;
DROP POLICY IF EXISTS "Staff can manage attorney SLA" ON public.attorney_sla;
DROP POLICY IF EXISTS "Users can view communications for their cases" ON public.client_communications;
DROP POLICY IF EXISTS "Staff can create communications" ON public.client_communications;
DROP POLICY IF EXISTS "Users can view retention policy" ON public.data_retention_policy;
DROP POLICY IF EXISTS "Admins can manage retention policy" ON public.data_retention_policy;
DROP POLICY IF EXISTS "Staff can view attorney performance" ON public.attorney_performance;
DROP POLICY IF EXISTS "System can update attorney performance" ON public.attorney_performance;
