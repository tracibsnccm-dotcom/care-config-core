...
import { isDemoMode } from "../lib/demoModeGuard";
...
const AttorneyConsole: React.FC = () => {
  ...
  // Use unified active case state - no local selectedClientId needed
  const { activeCase, cases, setActiveCaseById } = useActiveCase();
  ...
  // Use activeCase from unified state - map to selectedClient format for compatibility
  const selectedClient = useMemo(() => {
    if (!activeCase) {
      // Fallback to first case if no active case
      const firstCase = cases[0];
      if (!firstCase) return null;
      return {
        id: firstCase.id,
        displayName: firstCase.client.name || "Client",
        caseId: firstCase.id,
        injury: firstCase.client.notes || "Injury not specified",
        jurisdiction: "Unknown",
        lastUpdateISO: new Date().toISOString(),
        status: (firstCase.client.caseStatus || "Active") as ClientStatus,
        summary: {} as CaseSummary, // Will be loaded from storage if available
      };
    }
    
    // Convert activeCase to selectedClient format
    return {
      id: activeCase.id,
      displayName: activeCase.client.name || "Client",
      caseId: activeCase.id,
      injury: activeCase.client.notes || "Injury not specified",
      jurisdiction: "Unknown",
      lastUpdateISO: new Date().toISOString(),
      status: (activeCase.client.caseStatus || "Active") as ClientStatus,
      summary: {} as CaseSummary, // Will be loaded from storage if available
    };
  }, [activeCase, cases]);
  ...
  // When switching active case: only in Demo Mode
  useEffect(() => {
    if (!demoMode || !selectedClient) return;
    // Try to load summary from storage, fallback to empty
    const stored = loadCaseSummaryFromStorage();
    if (stored) {
      setSummary(stored);
    } else {
      setSummary(selectedClient.summary || null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeCase?.id, demoMode]);
  ...
  const toggleDemoMode = () => {
    // In demo mode context (/demo route), always keep demo mode ON
    // This toggle is only for internal testing, not for demo users
    if (isDemoMode()) {
      // If we're in demo route, don't allow disabling demo mode
      showToast("Demo mode cannot be disabled in demo environment.");
      return;
    }
    const next = !demoMode;
    setDemoMode(next);
  ...
  const combinedClinicalLabel = (() => {
    ...
    if (!clinicalWorst) return "Awaiting RN assessment";
    const label = getSeverityLabel(clinicalWorst);
    if (!label) return "Clinical assessment in progress";
    // Use prose description instead of raw score
    if (clinicalWorst <= 2) return "Significant clinical concerns identified";
    if (clinicalWorst <= 3) return "Moderate clinical concerns";
    return "Generally stable with routine monitoring";
  })();
  ...
  const buildClinicalNarrative = () => {
    ...
    // Use narrative-focused language instead of raw scores
    if (fourPs && fourPs.narrative) {
      lines.push(fourPs.narrative);
    } else if (fourPsOverall) {
      // Fallback to prose description if narrative not available
      if (fourPsOverall <= 2) {
        lines.push("The RN assessment indicates significant functional limitations and care needs across physical, psychological, psychosocial, and professional domains.");
      } else if (fourPsOverall <= 3) {
        lines.push("The RN assessment shows moderate care needs with some areas requiring attention and support.");
      } else {
        lines.push("The RN assessment indicates generally stable wellness with manageable care needs.");
      }
    } else {
      lines.push(
        "4Ps of Wellness assessment is pending; clinical domain evaluation is still in progress."
      );
    }

    if (tenVs && tenVs.narrative) {
      lines.push(tenVs.narrative);
    } else if (tenVsOverall) {
      // Fallback to prose description if narrative not available
      if (tenVsOverall <= 2) {
        lines.push("Care plan requires active reinforcement and closer monitoring. Treatment adherence and follow-through need consistent support.");
      } else if (tenVsOverall <= 3) {
        lines.push("Care plan is generally stable but may benefit from periodic reinforcement and coordination.");
      } else {
        lines.push("Care plan is well-maintained with good adherence and coordination.");
      }
    } else {
      lines.push(
        "10-Vs of Care Management assessment is pending; care plan evaluation is still in progress."
      );
    }

    if (sdoh && sdoh.narrative) {
      lines.push(sdoh.narrative);
    } else if (sdohOverall) {
      // Fallback to prose description if narrative not available
      if (sdohOverall <= 2) {
        lines.push("Significant social and environmental barriers are impacting care access and adherence. These barriers require attention and may affect case outcomes.");
      } else if (sdohOverall <= 3) {
        lines.push("Some social and environmental factors may impact care, but supports are generally present.");
      } else {
        lines.push("Social and environmental supports are generally strong and supportive of care adherence.");
      }
    } else {
      lines.push(
        "SDOH assessment is pending; social and environmental factors are still being evaluated."
      );
    }
    ...
    if (crisisSeverity) {
      if (crisisSeverity >= 4) {
        lines.push(
          "Crisis Mode has been activated, indicating acute safety or care concerns that require immediate attention and coordination."
        );
      } else {
        lines.push(
          "Some crisis-level concerns have been documented; ongoing monitoring and coordination are recommended."
        );
      }
    } else {
      lines.push(
        "No Crisis Mode events have been documented; no acute safety concerns are currently flagged."
      );
    }
  ...
  // Removed renderTenVsMiniList() and renderFourPsMiniList() - these show raw RN scoring details
  // Attorneys should not see individual dimension scores per CARE_SYSTEM_CONTEXT.md
  ...
  // Convert cases to client list format for display
  const clientList = useMemo(() => {
    return cases.map((c) => ({
      id: c.id,
      displayName: c.client.name || "Client",
      caseId: c.id,
      injury: c.client.notes || "Injury not specified",
      jurisdiction: "Unknown",
      lastUpdateISO: new Date().toISOString(),
      status: (c.client.caseStatus || "Active") as ClientStatus,
      summary: {} as CaseSummary,
    }));
  }, [cases]);
  ...
  const reports = useMemo(() => {
    ...
    return buildDemoReports(selectedClient, summary);
      ...
      >
        ...
        >
          ...
          <div style={{ marginTop: "0.9rem", paddingTop: "0.9rem", borderTop: "1px solid #e2e8f0" }}>
            ...
            >
              <strong>Demo Preview:</strong> This is a structured placeholder report. Reports are read-only in demo mode and do not navigate to external routes or trigger data writes. In live mode, reports will be generated from RN assessments.
            </div>
          </div>
        </div>
      </div>
    ...
    const renderLeftClientsPanel = () => {
      return (
        ...
        >
          ...
          {/* Case selector is always visible once demo is unlocked (per CARE_SYSTEM_CONTEXT.md) */}
          {cases.length === 0 ? (
            <div
              style={{
            ...
            >
              <div style={{ fontWeight: 900, marginBottom: "0.25rem" }}>No cases available</div>
              <div>No cases are currently available.</div>
            </div>
          ) : (
            <>
              <input
                ...
                {filteredSortedClients.map((c) => {
                  ...
                  return (
                    <button
                      key={c.id}
                      type="button"
                      onClick={() => setActiveCaseById(c.id)}
                      style={{
                        textAlign: "left",
                        padding: "0.7rem",
                        borderRadius: "12px",
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "baseline", flexWrap: "wrap" }}>
            ...
            <div style={{ fontSize: "0.85rem", color: "#64748b" }}>
              Selected: <strong>{activeCase?.id || "None"}</strong>
            </div>
          </div>
          ...
          {reports.length === 0 ? (
            ...
            >
              <div style={{ fontWeight: 900, marginBottom: "0.5rem", fontSize: "1rem" }}>No reports available</div>
              <div>
                Reports will appear here once RN assessments are completed for the selected case. This is a demo preview—reports are read-only placeholders.
              </div>
            </div>
          ) : (
              ...
              >
                <div style={{ borderRadius: "10px", border: "1px solid #e2e8f0", background: "#ffffff", padding: "0.85rem" }}>
                  ...
                  <div style={{ fontSize: "0.85rem", color: "#64748b" }}>
                    {demoMode ? (
                      <>
                        {fourPs && fourPs.narrative && <div>{fourPs.narrative}</div>}
                        {tenVs && tenVs.narrative && <div style={{ marginTop: "0.5rem" }}>{tenVs.narrative}</div>}
                        {crisisSeverity && (
                          <div style={{ marginTop: "0.5rem" }}>
                            {crisisSeverity >= 4 
                              ? "Crisis Mode active - acute concerns require immediate attention."
                              : "Some crisis-level concerns documented."}
                          </div>
                        )}
                        {!fourPs && !tenVs && !crisisSeverity && (
                          <div>RN assessment is pending. Clinical summary will appear once RN completes evaluation.</div>
                        )}
                      </>
                    ) : (
                ...
                <div style={{ borderRadius: "10px", border: "1px solid #e2e8f0", background: "#ffffff", padding: "0.85rem" }}>
                  ...
                  <div style={{ fontSize: "1rem", fontWeight: 700, marginBottom: "0.1rem" }}>
                    {demoMode
                      ? tenVsOverall
                        ? "Care Plan Status"
                        : "Awaiting 10-Vs assessment"
                      : "Live mode: awaiting onboarding"}
                  </div>
                  <div style={{ fontSize: "0.85rem", color: "#64748b" }}>
                    {demoMode ? (
                      tenVs && tenVs.narrative ? (
                        <span>{tenVs.narrative}</span>
                      ) : tenVsOverall ? (
                        <span>Care plan evaluation completed. See narrative for details on care coordination needs and recommended steps.</span>
                      ) : (
                        <span>Once the RN completes the 10-Vs assessment, you will see a care plan summary with coordination recommendations.</span>
                      )
                    ) : (
                ...
                <div style={{ borderRadius: "10px", border: "1px solid #e2e8f0", background: "#ffffff", padding: "0.85rem" }}>
                  ...
                  <div style={{ fontSize: "1rem", fontWeight: 700, marginBottom: "0.1rem" }}>
                    {demoMode ? (sdohOverall ? "Social & Environmental Factors" : "SDOH Assessment Pending") : "Live mode: awaiting onboarding"}
                  </div>
                  <div style={{ fontSize: "0.85rem", color: "#64748b" }}>
                    {demoMode ? (
                      sdoh && sdoh.narrative ? (
                        <span>{sdoh.narrative}</span>
                      ) : (
          ...
          return (
            <div style={{ borderRadius: "10px", border: "1px solid #e2e8f0", background: "#ffffff", padding: "0.95rem" }}>
              ...
              {narrativeLines.length === 0 ? (
                <div style={{ fontSize: "0.88rem", color: "#64748b", padding: "0.75rem", borderRadius: "8px", background: "#f8fafc", border: "1px solid #e2e8f0" }}>
                  No clinical narrative available yet. RN assessments are pending.
                </div>
              ) : (
              ...
              {!demoMode ? (
                ...
                <>
                  <ul style={{ fontSize: "0.88rem", color: "#64748b", paddingLeft: "1.1rem", marginBottom: "0.35rem" }}>
                    {sdohOverall && (
                      <li>
                        <strong>Social & Environmental Factors:</strong> {sdohOverall <= 2 ? "Significant barriers present" : sdohOverall <= 3 ? "Moderate barriers" : "Generally supportive"}
                      </li>
                    )}
                    {fourPsOverall && (
                      <li>
                        <strong>Wellness Assessment:</strong> {fourPsOverall <= 2 ? "Significant care needs" : fourPsOverall <= 3 ? "Moderate care needs" : "Generally stable"}
                      </li>
                    )}
                    {tenVsOverall && (
                      <li>
                        <strong>Care Plan Status:</strong> {tenVsOverall <= 2 ? "Requires reinforcement" : tenVsOverall <= 3 ? "Stable with monitoring" : "Well-maintained"}
                      </li>
                    )}
                    {crisisSeverity && (
                      <li>
                        <strong>Crisis Mode:</strong> {crisisSeverity >= 4 ? "Active - acute concerns" : "Some concerns documented"}
                      </li>
                    )}
                    {!sdohOverall && !fourPsOverall && !tenVsOverall && !crisisSeverity && (
                      <li>RN assessment is pending. Summary will appear once evaluation is complete.</li>
                    )}
                  </ul>
    ...
    return (
      <div>
        ...
        >
          ...
          {/* Case selector is always visible once unlocked (per CARE_SYSTEM_CONTEXT.md) */}
          {cases.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
              ...
              <select
                value={activeCase?.id || ""}
                onChange={(e) => setActiveCaseById(e.target.value)}
                style={{
                  border: "1px solid #cbd5e1",
                  borderRadius: "10px",
  ...
  const headerCenterText = activeCase
    ? `${activeCase.client.name || "Client"} • ${activeCase.id}`
    : "No case selected";
    ...
    <div style={{ background: "#f8fafc", minHeight: "100vh" }}>
      ...
      >
        ...
        >
          ...
          {/* Settings hidden in demo mode - out of scope per CARE_SYSTEM_CONTEXT.md */}
          {!isDemoMode() && (
