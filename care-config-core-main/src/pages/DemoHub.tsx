...
import { DEMO_ACCESS_CODE } from "../config/demoAccess";
...
const DemoHub: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (v === "true") {
        ...
        // If not unlocked, ensure view is reset to hub (gate screen)
        setUnlocked(false);
        setView("hub");
      }
    } catch {
  ...
  const lock = () => {
    ...
    setView("hub"); // Reset to hub (gate screen) when locking
    setCode("");
    setError(null);
  };
  ...
  const resetDemo = () => {
    ...
    setCode("");
    setError(null);
    // Only redirect if not already on /demo
  ...
  // CRITICAL: Always show gate if not unlocked, regardless of view state
  // This prevents bypassing the gate by manipulating view state
  if (!unlocked) {
    // Force view to hub and show gate - ignore any other view state
    // (gate UI is rendered below in the hub view)
  } else if (unlocked && view === "attorney") {
    return (
      <div style={ui.page}>
  ...
  // --- Hub view (gate screen when locked, hub when unlocked) ---
  // This is always shown if not unlocked, or if view is "hub"
  // CRITICAL: Gate enforcement - if not unlocked, this is the only view shown
  if (!unlocked) {
    // Force view to hub when locked - ignore any other view state
    if (view !== "hub") {
      setView("hub");
    }
  }
